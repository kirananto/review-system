...
// GetProviderHotels godoc
// @Summary Get all provider hotels
// @Description Get all provider hotels
// @ID get-provider-hotels
// @Produce json
// @Success 200 {array} review.ProviderHotel
// @Router /provider-hotels [get]
func (h *ProviderHotelHandler) GetProviderHotels(w http.ResponseWriter, r *http.Request) {
	providerHotels, err := h.service.GetProviderHotels(r.Context())
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(providerHotels)
}
